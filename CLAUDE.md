...
- `com.backend.jobportal.security` holds the Spring Security setup (`spring-boot-starter-security` + `jjwt`):
  - `PathConfig` defines three named `List<String>` beans consumed by qualifier — `publicPaths` (exact/ant paths, e.g. Swagger/OpenAPI UI, `/auth/login/public`), `securedPaths` (currently `/api/**`), and `regexPaths` (regex-matched paths, e.g. `.*public$` so any endpoint ending in `public` is open). Add new open endpoints to one of these lists rather than editing the filter chain directly.
  - `JobPortalSecurityConfig` wires those beans into the `SecurityFilterChain`: CSRF disabled, its own CORS source (currently duplicating `http://localhost:5173`), regex/public paths permitted, secured paths requiring authentication, form login disabled. `security.filter.JwtTokenValidatorFilter` runs before `BasicAuthenticationFilter`, skipping public/regex paths and otherwise reading the `Authorization` header (raw JWT after stripping 7 chars, no `Bearer ` prefix check), verifying it with `security.util.JwtUtil` against an HMAC secret (`JWT_SECRET` env var, falling back to a hardcoded default in `constants.ApplicationConstant` — replace before any real deployment), and populating `SecurityContextHolder` from the token's `username`/`roles` claims. HTTP Basic auth (`httpBasic(withDefaults())`) is also still enabled as a fallback auth mechanism, backed by an in-memory `UserDetailsService` (`JobPortalSecurityConfig.userDetailsService`) with two hardcoded users (`Anurag`/`ADMIN`) — there is no database-backed user store yet, so `AuditAwareImpl` remains a stub.
  - Login flow: `POST /api/auth/login/public` (`auth.controller.AuthController`) takes a `LoginRequestDto` (username/password), authenticates via `AuthenticationManager`, and on success returns a `LoginResponseDto` containing a JWT from `JwtUtil.generateJwtToken` (24h expiry). Send that token back as the raw `Authorization` header value (not prefixed with `Bearer `, since the filter only strips a fixed 7 characters) on subsequent requests to secured paths.
