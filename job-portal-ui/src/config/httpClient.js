 ...
 * Follows the vendor-specific media type format: application/vnd.backend+json;v=1.0
 * Developers can override this in individual requests if needed
 */
export const DEFAULT_ACCEPT_HEADER = "application/vnd.backend+json;v=1.0";
 ...
 * const response = await httpClient.get('/companies', {
 *   headers: { 'Accept': 'application/vnd.backend+json;v=2.0' }
 * });
 *
 * @example
...
export const withApiVersion = (version) => {
  const acceptHeader = `application/vnd.backend+json;v=${version}`;
  return {
    get: (url, config = {}) =>
      httpClient.get(url, {
        ...config,
